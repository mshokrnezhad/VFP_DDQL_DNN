...
class P3ADDPG:
    ...
    def solve(self):
        ...
        for r in self.req_obj.REQUESTS:
            ...
            u_re = self.req_obj.BW_REQUIREMENTS[r] * 1000   
            flag_node = False
            max_u = 0
            for v in self.net_obj.NODES:
                d_node = self.net_obj.PACKET_SIZE / self.req_obj.CAPACITY_REQUIREMENTS[r]
                    ...
                    if(len(req_paths) > 0 and len(rpl_paths) > 0):
                        ...
                        for p1 in req_paths:
                            ...
                            for p2 in rpl_paths:
                                ...
                                d_path = 0
                                for l1 in np.where(self.net_obj.LINKS_PATHS_MATRIX[:, p1] == 1)[0]:
                                    d_path += self.net_obj.LINK_DELAYS[l1][k]
                                for l2 in np.where(self.net_obj.LINKS_PATHS_MATRIX[:, p2] == 1)[0]:
                                    d_path += self.net_obj.LINK_DELAYS[l2][k]
                                    
                                d = d_node + d_path
                                u = u_re + (1/d)*math.log(1/d)
